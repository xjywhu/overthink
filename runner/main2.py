...
def extract_useful_content(content: str) -> str:
    return content.split("<useful_content>")[-1].split("</useful_content>")[0].strip()
...
def extract_input_prompt(content: str) -> str:
    if "Do NOT output any extra information." in content:
        content = content.split("Do NOT output any extra information.")[0]
    elif "Ensure the provided expression syntax is correct!" in content:
        content = content.split("Ensure the provided expression syntax is correct!")[0]
...
class Runner2:
    def __init__(self) -> None:
        self.lock = Lock()
        self.model = OpenKeyChat("gpt-4o-mini", temperature=0, top_p=0.95, max_tokens=10000, delay=0)
    ...
    def run_demo(self, question: dict) -> str:
        ...
        inputs = {
            "task": extract_input_prompt(question["input_prompt"]),
            "reasoning_content": think,
        }
        prompt = FILTER_PROMPT.format(**inputs).strip()
        ...
        try:
            extracted_response = extract_useful_content(response[0])
    ...
    def start(self, filepath: str, max_workers: int = 30, **kwargs) -> None:
        ...
        def process_function(idx, _):
            try:
                ...
                if "useful_content" in question:
                    return

                response = self.run_demo(question)
                if response:
                    question["useful_content"] = response
                    with self.lock:
                        self.save()
                else:
